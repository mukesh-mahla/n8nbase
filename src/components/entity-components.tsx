import { AlertTriangleIcon, Divide, Loader2Icon, MoreVerticalIcon, PackageOpenIcon, PlusIcon, SearchIcon, TrashIcon } from "lucide-react";
...
export const EntityPagination = ({page,onPageChange,totalPages,diabled}:EntityPaginationProps)=>{
    ...
    return <div className="flex items-center justify-between gap-x-2 w-full">
        <div className="flex-1 text-sm text-muted-foreground">
         ...
         <div className="flex items-center justify-end space-x-2 py-4">
            <Button disabled={page === 1 || diabled}
            variant={"outline"}
            size={"sm"}
            onClick={()=>onPageChange(Math.max(1,page - 1))}>
                Previous
            </Button>
             <Button disabled={page === totalPages || totalPages === 0 || diabled}
            variant={"outline"}
            size={"sm"}
...
export const LoadingView = ({message}: StateViewProps)=>{
 return <div className="flex justify-center items-center flex-1 flex-col h-full gap-y-4">
...
export function EntityList<T>({items,renderItem,getKey,emptyView,className}:EntityListProps<T>){
    ...
    return <div className={cn("flex flex-col gap-y-4",className )}>
...
export const EntityItems = ({href,title,subtitle,image,action,onRemove,isRemoving,className}:EntityItemProps)=>{
